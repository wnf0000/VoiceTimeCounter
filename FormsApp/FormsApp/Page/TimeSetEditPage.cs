...
{
    class TimeSetEditPage : ContentPage
    {
        ...
        async void saveButton_Clicked(object sender, EventArgs e)
        {
            ...
            TimeSet.Name = ViewModel.Name;
            TimeSet.Duration = ViewModel.Duration;
            TimeSet.SecendsToReady = ViewModel.SecendsToReady;
            TimeSet.VoiceEngine = ViewModel.VoiceEngine;
            TimeSet.StagesDataString = ViewModel.StagesDataString;
            TimeSet.ObservableStages.Clear();
            ViewModel.ObservableStages.ForEach(TimeSet.AddStage);

            var result = await DbHelper.UpdateTimeSet(TimeSet) > 0;
            uds.HideLoading();
            toast.Show("保存成功");
           // uds.Toast("保存成功", 2);
                ...
                removeBtn.GestureRecognizers.Add(new TapGestureRecognizer()
                {
                    Command = new Command(() =>
                    {
                        ...
                        if (timeSet.ObservableStages.Count == 1)
                        {
                            ...
                            {
                                Message = "至少保留一个阶段",OkText = "好的",Title = "提示"
                            });
                            return;
                        }
